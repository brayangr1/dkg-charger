...
router.get('/:id/secondary-device', authenticate, async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    const [chargerExists] = await connectionPool.query<RowDataPacket[]>(
      'SELECT id FROM chargers WHERE id = ?',
      [chargerId]
    );
    ...
    const [rows] = await connectionPool.query<RowDataPacket[]>(
      'SELECT device_status FROM secondary_devices WHERE charger_id = ?',
      [chargerId]
    );
...
router.post('/:id/secondary-device', authenticate, async (req: AuthenticatedRequest, res) => {
  const chargerId = req.params.id;
  ...
  try {
    ...
    const [result]: any = await connectionPool.query(
      'UPDATE secondary_devices SET device_status = ? WHERE charger_id = ?',
      [device_status, chargerId]
    );
