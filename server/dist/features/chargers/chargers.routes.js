...
const adm_config_1 = require("../../config/adm.config");
...
router.post('/add', auth_1.authenticate, async (req, res) => {
    ...
    try {
        // Obtener conexiones
        connMain = await db_config_1.connectionPool.getConnection();
        connDevice = await deviceDb_config_1.deviceDbPool.getConnection();
        adminConn = await (0, adm_config_1.getAdminConnection)();
        // PASO 1: Verificar si ya existe en nuestra base
        const [existingChargers] = await connMain.query('SELECT id, owner_id FROM chargers WHERE serial_number = ?', [serial]);
        ...
        if (existingChargers.length > 0) {
            ...
            await connMain.query('UPDATE chargers SET owner_id = ?, last_updated = NOW() WHERE id = ?', [userId, chargerId]);
            ...
            await connMain.query(`INSERT INTO charger_users (charger_id, user_id, access_level, rate_per_kwh) 
                 VALUES (?, ?, 'owner', ?)
                 ON DUPLICATE KEY UPDATE access_level = 'owner'`, [chargerId, userId, basePricePerKwh]);
            // 3. Asegurar dispositivo secundario
            await connMain.query('INSERT IGNORE INTO secondary_devices (charger_id, device_status) VALUES (?, ?)', [chargerId, false]);
        ...
        const [deviceRows] = await connDevice.query('SELECT * FROM devices WHERE serial = ?', [serial]);
        ...
        if (chargerType === 'dkg') {
            try {
                ...
                const [priceRows] = await deviceDb_config_1.deviceDbPool.query('SELECT base_price_per_kwh FROM administracion.pricing_devices WHERE serial_number = ?', [serial]);
        ...
        let groupId = null;
        if (chargerType === 'dkg') {
            const [groupRows] = await connDevice.query('SELECT * FROM groups WHERE owner_email = ?', [userEmail]);
            if (groupRows.length == 0) {
                console.log(`Creando nuevo grupo para ${userEmail}`);
                const [groupResult] = await connDevice.query('INSERT INTO groups (name, owner_email) VALUES (?, ?)', [`Grupo de ${userEmail.split('@')[0]}`, userEmail]);
            ...
            if (!device.group || device.group !== groupId) {
                ...
                await connDevice.query('UPDATE devices SET `group` = ? WHERE id = ?', [groupId, device.id]);
            }
        }
        ...
        const chargerData = {
            ...
            owner_id: userId,
            charger_type: chargerType // ← NUEVO CAMPO IMPORTANTE
        };
        console.log(`Insertando cargador: ${JSON.stringify(chargerData)}`);
        const [insertResult] = await connMain.query('INSERT INTO chargers SET ?', [chargerData]);
        ...
        await connMain.query('INSERT INTO secondary_devices (charger_id, device_status) VALUES (?, ?)', [chargerId, false]);
        ...
        await connMain.query('INSERT INTO charger_users (charger_id, user_id, access_level, rate_per_kwh) VALUES (?, ?, ?, ?)', [chargerId, userId, 'owner', basePricePerKwh]);
        // Actualizar estado de invitado si era invitado
        await chargerService_1.chargerService.updateGuestStatus(userId, false);
        ...
        if (chargerType === 'dkg') {
            ...
            console.log(`Creando tablas dinámicas: ${logTable} y ${actionTable}`);
        ...
        if (connDevice)
            connDevice.release();
        if (adminConn)
            adminConn.release(); // Liberar conexión a administracion
    }
});
// Ruta para obtener listas dispositivos
router.get('/list', async (req, res) => {
    try {
        const [devices] = await deviceDb_config_1.deviceDbPool.query('SELECT serial, name FROM devices');
        res.json(devices);
    }
    catch (error) {
...
router.post('/:serial/claim', auth_1.authenticate, async (req, res) => {
    ...
    try {
        ...
        const [existingChargers] = await conn.query('SELECT id, owner_id, serial_number FROM chargers WHERE serial_number = ?', [serial]);
        ...
        if (existingChargers.length > 0 && !existingChargers[0].owner_id) {
            ...
            await conn.query('UPDATE chargers SET owner_id = ?, last_updated = NOW() WHERE id = ?', [userId, chargerId]);
        ...
        // PASO 3: Si no existe, crear nuevo cargador
            ...
            const [insertResult] = await conn.query('INSERT INTO chargers SET ?', [chargerData]);
            chargerId = insertResult.insertId;
            console.log(`Nuevo cargador ${serial} creado para usuario ${userId}`);
        }
        // PASO 4: Crear registro en charger_users como owner
        await conn.query(`INSERT INTO charger_users (charger_id, user_id, access_level, rate_per_kwh) 
       VALUES (?, ?, ?, ?) 
       ON DUPLICATE KEY UPDATE access_level = VALUES(access_level)`, [chargerId, userId, 'owner', 0.30] // Precio por defecto
        );
        // PASO 5: Crear dispositivo secundario si no existe
        const [secondaryDevices] = await conn.query('SELECT id FROM secondary_devices WHERE charger_id = ?', [chargerId]);
        if (secondaryDevices.length === 0) {
            await conn.query('INSERT INTO secondary_devices (charger_id, device_status) VALUES (?, ?)', [chargerId, false]);
        }
        return res.json({
            success: true,
...
router.put('/:serial/associate', auth_1.authenticate, async (req, res) => {
    ...
    try {
        ...
        const [rows] = await conn.query('SELECT id, owner_id FROM chargers WHERE serial_number = ?', [serial]);
        ...
        await conn.query('INSERT INTO charger_users (charger_id, user_id, access_level, rate_per_kwh) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE access_level = VALUES(access_level)', [charger.id, userId, 'owner', 0]);
...
router.get('/status/:serial', async (req, res) => {
    ...
    try {
        conn = await db_config_1.connectionPool.getConnection();
        const [rows] = await conn.query('SELECT id, owner_id, network_status, status, last_updated FROM chargers WHERE serial_number = ?', [serial]);
        if (!rows || rows.length === 0) {
            return res.status(404).json({ success: false, error: 'Cargador no encontrado' });
        }
        const c = rows[0];
        ...
        let owner = null;
        if (c.owner_id) {
            const [owners] = await conn.query('SELECT id, email, name FROM users WHERE id = ?', [c.owner_id]);
...
router.post('/pricing', async (req, res) => {
    ...
    try {
        ...
        await deviceDb_config_1.deviceDbPool.query(`
      ...
      ON DUPLICATE KEY UPDATE 
        ...
        updated_at = NOW()
    `, [serial_number, basePrice, gridPrice, price_solar, price_mixed, price_surplus]);
        // 2. Actualizar directamente en charger_users (sin esperar sincronización)
        const [result] = await db_config_1.connectionPool.query(`UPDATE charger_users cu
       JOIN chargers c ON cu.charger_id = c.id
       SET cu.rate_per_kwh = ?
       WHERE c.serial_number = ? 
       AND cu.access_level = 'owner'`, [gridPrice, serial_number]);
        ...
        return res.json({
            ...
            updatedInChargers: result.affectedRows > 0
        });
    }
    catch (error) {
...
router.get('/mine-with-owner', auth_1.authenticate, async (req, res) => {
    const userId = req.user?.id;
    try {
      ...
      WHERE cu.user_id = ?`, [userId, userId, userId, userId]);
...
router.get('/mine', auth_1.authenticate, async (req, res) => {
    ...
    try {
      ...
      WHERE cu.user_id = ?`, [userId, userId, userId]);
        // Procesar los resultados para usar el estado de primary_devices si existe
        const processedRows = rows.map(row => {
            // Si hay un estado en primary_devices, usar ese, si no, usar el estado original
            if (row.primary_device_status !== null && row.primary_device_status !== undefined) {
...
router.get('/:id', auth_1.authenticate, async (req, res) => {
    ...
    try {
        ...
        const [accessRows] = await db_config_1.connectionPool.query('SELECT access_level FROM charger_users WHERE charger_id = ? AND user_id = ?', [chargerId, userId]);
        ...
        const [chargerRows] = await db_config_1.connectionPool.query(`SELECT 
                c.*,
                cu.access_level,
                cu.rate_per_kwh,
                cu.energy_limit
            ...
            WHERE c.id = ? AND cu.user_id = ?`, [chargerId, userId]);
        ...
        const [schedules] = await db_config_1.connectionPool.query('SELECT * FROM charging_schedules WHERE charger_id = ?', [chargerId]);
        ...
        const [sessionsEnergy] = await db_config_1.connectionPool.query('SELECT COALESCE(SUM(total_energy), 0) as total FROM charging_sessions WHERE charger_id = ? AND start_time >= ?', [chargerId, firstDayOfMonth]);
        const [usersEnergy] = await db_config_1.connectionPool.query('SELECT COALESCE(SUM(monthly_energy_used), 0) + COALESCE(SUM(monthly_energy_accumulated), 0) as total FROM charger_users WHERE charger_id = ?', [chargerId]);
        ...
        const [debugSessions] = await db_config_1.connectionPool.query('SELECT COUNT(*) as session_count, SUM(total_energy) as total_energy FROM charging_sessions WHERE charger_id = ?', [chargerId]);
        const [debugUsers] = await db_config_1.connectionPool.query('SELECT COUNT(*) as user_count, SUM(monthly_energy_used) as total_used, SUM(monthly_energy_accumulated) as total_accumulated FROM charger_users WHERE charger_id = ?', [chargerId]);
        ...
        const [sessions] = await db_config_1.connectionPool.query(`SELECT 
                id, 
                start_time, 
                end_time,
                total_energy, 
...
router.put('/:id/settings', auth_1.authenticate, async (req, res) => {
    const chargerId = req.params.id;
    const userId = req.user?.id;
...
router.get('/:id/settings', auth_1.authenticate, async (req, res) => {
    const chargerId = req.params.id;
    const userId = req.user?.id;
    try {
        // Verificar acceso
        ...
        const [chargerRows] = await db_config_1.connectionPool.query('SELECT name, wifi_ssid, lighting_config, lock_methods FROM chargers WHERE id = ?', [chargerId]);
        if (chargerRows.length === 0) {
...
router.put('/:id/power', auth_1.authenticate, async (req, res) => {
    const chargerId = req.params.id;
    ...
    try {
        ...
        await db_config_1.connectionPool.query('UPDATE chargers SET max_power = ? WHERE id = ?', [max_power, chargerId]);
        const [updated] = await db_config_1.connectionPool.query('SELECT * FROM chargers WHERE id = ?', [chargerId]);
        res.json({
            success: true,
...
router.put('/:id/plug', auth_1.authenticate, async (req, res) => {
    const chargerId = req.params.id;
    ...
    try {
        ...
        await db_config_1.connectionPool.query('UPDATE chargers SET status = ? WHERE id = ?', [newStatus, chargerId]);
        console.log(`PlugControl - Estado actualizado correctamente en la base de datos`);
        // Registrar la acción en los logs
        await db_config_1.connectionPool.query('INSERT INTO logs (charger_id, action, description, executed_at) VALUES (?, ?, ?, NOW())', [chargerId, is_plugged ? 'plug_in' : 'plug_out', `Usuario ${userId} ${is_plugged ? 'activó' : 'desactivó'} el cargador`]);
        res.json({
            success: true,
...
//**  * POST /api/chargers/:id/schedules  * Agrega una nueva programación de carga  */
// Este endpoint permite a un usuario autenticado agregar una nueva programación de carga para un cargador específico al que tiene acceso.
router.post('/:id/schedules', auth_1.authenticate, async (req, res) => {
    ...
    try {
        // Insertar en BD
        const [result] = await db_config_1.connectionPool.query('INSERT INTO charging_schedules (charger_id, schedule_name, start_time, end_time, week_days, action, power) VALUES (?, ?, ?, ?, ?, ?, ?)', [chargerId, schedule_name, start_time, end_time, week_days.join(','), action, power || null]);
        // Enviar notificación de programación creada
        await notificationService_1.notificationService.sendScheduleCreatedNotification(chargerId, schedule_name, start_time, end_time);
        res.json({
            success: true,
            scheduleId: result.insertId,
            message: 'Programación agregada correctamente'
        });
    }
    catch (error) {
        ...
        res.status(500).json({
            success: false,
            error: 'Error al agregar programación'
        });
    }
});
//  * DELETE /api/chargers/:id/schedules/:scheduleId  * Elimina una programación de carga
// Este endpoint permite a un usuario autenticado eliminar una programación de carga específica de un cargador al que tiene acceso.
// Requiere autenticación y verifica que el usuario tenga permisos para eliminar la programación.
// Si el usuario no tiene permisos (por ejemplo, si es un usuario invitado), devuelve un error 403.
// Si la programación no existe, devuelve un error 404. Si todo es correcto, elimina la programación de la base de datos y devuelve un mensaje de éxito.
...
router.delete('/:id/schedules/:scheduleId', auth_1.authenticate, async (req, res) => {
    ...
    try {
        ...
        // Obtener información de la programación antes de eliminarla
        const [scheduleInfo] = await db_config_1.connectionPool.query('SELECT schedule_name FROM charging_schedules WHERE id = ? AND charger_id = ?', [scheduleId, chargerId]);
        ...
        await db_config_1.connectionPool.query('DELETE FROM charging_schedules WHERE id = ? AND charger_id = ?', [scheduleId, chargerId]);
        // Enviar notificación de programación eliminada
        await notificationService_1.notificationService.sendScheduleDeletedNotification(Number(chargerId), scheduleInfo[0].schedule_name);
        res.json({
            success: true,
...
router.put('/:id/lock', auth_1.authenticate, async (req, res) => {
    const chargerId = Number(req.params.id); // Convertir a número
    ...
    try {
        ...
        await db_config_1.connectionPool.query('INSERT INTO logs (charger_id, action, description, executed_at) VALUES (?, ?, ?, NOW())', [chargerId, is_locked ? 'lock' : 'unlock', `Usuario ${userId} ${is_locked ? 'bloqueó' : 'desbloqueó'} el cargador`]);
        // Notificar a los clientes WebSocket
        const [charger] = await db_config_1.connectionPool.query('SELECT network_status FROM chargers WHERE id = ?', [chargerId]);
        if (charger.length > 0) {
            app_1.webSocketServer.notifyStatusChange(chargerId, newStatus, charger[0].network_status);
        }
        // Enviar notificación push
        notificationService_1.notificationService.sendStatusChangeNotification(chargerId, newStatus);
        res.json({
            success: true,
...
router.put('/:id/lighting', auth_1.authenticate, async (req, res) => {
    ...
    try {
        ...
        // Actualizar en base de datos
        await db_config_1.connectionPool.query('UPDATE chargers SET lighting_config = ? WHERE id = ?', [JSON.stringify(lighting), chargerId]);
...
async function sendCommandToDevice(chargerId, command, data) {
    // 1. Obtener serial_number desde la base principal (charger)
    const [chargerRows] = await db_config_1.connectionPool.query('SELECT serial_number FROM chargers WHERE id = ?', [chargerId]);
    if (chargerRows.length === 0)
...
router.put('/:id/charging-mode', auth_1.authenticate, async (req, res) => {
    const chargerId = req.params.id;
    const { mode } = req.body;
    const userId = req.user?.id;
    ...
    try {
        ...
        // Actualizar en base de datos
        await db_config_1.connectionPool.query('UPDATE chargers SET charging_mode = ? WHERE id = ?', [mode, chargerId]);
        // Enviar comando al dispositivo físico (simulado)
        await sendCommandToDevice(Number(chargerId), 'set_charging_mode', { mode: mode });
        res.json({
            success: true,
...
router.get('/:id/current-session', auth_1.authenticate, async (req, res) => {
    const chargerId = req.params.id;
    const userId = req.user?.id;
    try {
        // Verificar acceso
        ...
        const [charger] = await db_config_1.connectionPool.query('SELECT serial_number FROM chargers WHERE id = ?', [chargerId]);
        if (charger.length === 0) {
            return res.status(404).json({
                success: false,
                error: 'Cargador no encontrado'
        ...
        const [currentSession] = await deviceDb_config_1.deviceDbPool.query(`SELECT * FROM charging_log_${charger[0].serial_number} 
       ...
       ORDER BY start_time DESC LIMIT 1`);
        ...
        const [userRate] = await db_config_1.connectionPool.query('SELECT rate_per_kwh FROM charger_users WHERE charger_id = ? AND user_id = ?', [chargerId, userId]);
...
router.post('/:id/start-session', auth_1.authenticate, async (req, res) => {
    const chargerId = Number(req.params.id);
    const userId = req.user?.id;
    ...
    try {
        ...
        const [access] = await db_config_1.connectionPool.query(`SELECT access_level, rate_per_kwh 
       FROM charger_users 
       WHERE charger_id = ? AND user_id = ?`, [chargerId, userId]);
        if (access.length === 0) {
            return res.status(403).json({
                success: false,
                error: 'No tienes acceso a este cargador'
        ...
        const [charger] = await db_config_1.connectionPool.query(`SELECT status, network_status, serial_number 
       FROM chargers 
       WHERE id = ?`, [chargerId]);
        if (charger.length === 0) {
            return res.status(404).json({
        ...
        const isValid = await paymentService_1.paymentService.validatePaymentMethod(userId, paymentMethodId);
        if (!isValid) {
        ...
        try {
            ...
            const [sessionResult] = await conn.query(`INSERT INTO charging_sessions 
         (charger_id, user_id, start_time, charging_mode, payment_method_id, rate_per_kwh) 
         VALUES (?, ?, NOW(), 'grid', ?, ?)`, [chargerId, userId, paymentMethodId, ratePerKwh]);
            const sessionId = sessionResult.insertId.toString();
            // Crear en charging_log_XXX del dispositivo
            await deviceDb_config_1.deviceDbPool.query(`INSERT INTO charging_log_${charger[0].serial_number} 
         (start_time, rate_per_kwh) 
         VALUES (NOW(), ?)`, [ratePerKwh]);
            ...
            await conn.query(`UPDATE chargers 
         SET status = 'charging' 
         WHERE id = ?`, [chargerId]);
            // 7. Registrar en logs
            await conn.query(`INSERT INTO logs 
         (charger_id, action, description) 
         VALUES (?, ?, ?)`, [chargerId, 'start_charging', `Usuario ${userId} inició sesión (Tarifa: ${ratePerKwh}/kWh)`]);
            await conn.commit();
            // 8. Notificar por WebSocket
            app_1.webSocketServer.notifyStatusChange(chargerId, 'charging', 'online', {
                ratePerKwh
            });
            // 9. Enviar notificación de carga iniciada
            const [userInfo] = await db_config_1.connectionPool.query('SELECT name FROM users WHERE id = ?', [userId]);
            const userName = userInfo[0]?.name || 'Usuario';
            await notificationService_1.notificationService.sendManualChargingStartedNotification(chargerId, userName);
            res.json({
                success: true,
                sessionId,
                ratePerKwh,
...
router.post('/sessions/:id/stop', auth_1.authenticate, async (req, res) => {
    const sessionId = req.params.id;
    const userId = req.user?.id;
    try {
        console.log(`[STOP-LOG] Iniciando detención para sesión: ${sessionId}, por usuario: ${userId}`);
        // 1. Obtener información de la sesión
        const [session] = await db_config_1.connectionPool.query(`SELECT 
         cs.id, 
         cs.charger_id, 
         cs.start_time, 
         cs.user_id,
       ...
       WHERE cs.id = ? AND cs.end_time IS NULL`, [sessionId]);
        if (session.length === 0) {
        ...
        const [chargingData] = await deviceDb_config_1.deviceDbPool.query(`SELECT 
         energy_kwh, 
         power_peak 
       ...
       ORDER BY start_time DESC LIMIT 1`);
        ...
        const [rateRow] = await db_config_1.connectionPool.query(`SELECT rate_per_kwh 
       FROM charger_users 
       WHERE charger_id = ? AND user_id = ?`, [chargerId, userId]);
        ...
        await db_config_1.connectionPool.query(`UPDATE charging_sessions 
       SET 
         end_time = ?,
         total_energy = ?,
         duration_seconds = ?,
         estimated_cost = ?
       WHERE id = ?`, [endTime, energyConsumed, durationSeconds, totalCost, sessionId]);
        ...
        const [deviceLogResult] = await deviceDb_config_1.deviceDbPool.query(`UPDATE \`charging_log_${serialNumber}\`
       SET 
         end_time = ?,
         energy_kwh = ?,
         power_peak = ?
       WHERE end_time IS NULL
       ORDER BY start_time DESC LIMIT 1`, [endTime, energyConsumed, chargingData[0].power_peak || 0]);
       ...
       WHERE id = ?`, [chargerId]);
        // 8. Registrar en logs
        await db_config_1.connectionPool.query(`INSERT INTO logs 
       (charger_id, action, description) 
       VALUES (?, ?, ?)`, [chargerId, 'stop_charging', `Usuario ${userId} detuvo sesión de carga`]);
        // 9. Notificar por WebSocket
        app_1.webSocketServer.notifyStatusChange(chargerId, 'standby', 'online');
        // 10. Enviar notificación de carga finalizada
        const [userInfo] = await db_config_1.connectionPool.query('SELECT name FROM users WHERE id = ?', [userId]);
        const userName = userInfo[0]?.name || 'Usuario';
        await notificationService_1.notificationService.sendChargingStoppedNotification(chargerId, userName, energyConsumed, totalCost, durationSeconds);
        res.json({
            success: true,
...
router.get('/:id/monthly-energy', auth_1.authenticate, async (req, res) => {
    const chargerId = req.params.id;
    const userId = req.user?.id;
    const currentDate = new Date();
    const firstDayOfMonth = new Date(currentDate.getFullYear(), currentDate.getMonth(), 1);
    try {
        ...
        const [energyData] = await db_config_1.connectionPool.query(`SELECT SUM(total_energy) as monthly_energy 
             FROM charging_sessions 
             WHERE charger_id = ? 
             AND user_id = ?
             AND start_time >= ?`, [chargerId, userId, firstDayOfMonth]);
        res.json({
            success: true,
...
router.get('/:id/energy-settings', auth_1.authenticate, async (req, res) => {
    const chargerId = req.params.id;
    const userId = req.user?.id;
    try {
        const [chargerRows] = await db_config_1.connectionPool.query('SELECT charging_mode, serial_number FROM chargers WHERE id = ?', [chargerId]);
        if (chargerRows.length === 0) {
            return res.status(404).json({ error: 'Cargador no encontrado' });
        }
        const serial = chargerRows[0].serial_number;
        const [userRows] = await db_config_1.connectionPool.query('SELECT rate_per_kwh FROM charger_users WHERE charger_id = ? AND user_id = ?', [chargerId, userId]);
        // Obtener precios por modo de administracion.pricing_devices
        const [priceRows] = await deviceDb_config_1.deviceDbPool.query('SELECT base_price_per_kwh, price_grid, price_solar, price_mixed, price_surplus FROM administracion.pricing_devices WHERE serial_number = ?', [serial]);
        res.json({
...
// Opciones de fábrica
// --- RESET ---
router.post('/:chargerId/factory/reset', auth_1.authenticate, async (req, res) => {
    const chargerId = req.params.chargerId;
    try {
        // Lógica real: poner el estado en 'standby' y actualizar last_updated
        await db_config_1.connectionPool.query('UPDATE chargers SET status = ?, last_updated = NOW() WHERE id = ?', ['standby', chargerId]);
...
// --- RESTORE ---
router.post('/:chargerId/factory/restore', auth_1.authenticate, async (req, res) => {
    const chargerId = req.params.chargerId;
    try {
        // Lógica real: restaurar valores por defecto (ajusta según tus defaults)
        await db_config_1.connectionPool.query('UPDATE chargers SET name = ?, max_power = 32, status = ?, last_updated = NOW() WHERE id = ?', ['Cargador', 'standby', chargerId]);
...
router.get('/:chargerId/ping', auth_1.authenticate, async (req, res) => {
    const chargerId = req.params.chargerId;
    const userId = req.user?.id;
    try {
        ...
        const [charger] = await db_config_1.connectionPool.query(`SELECT 
        c.id,
        c.status,
        c.network_status,
        c.last_updated,
      ...
      WHERE c.id = ?`, [chargerId]);
        ...
        if (!isAlive && charger[0].network_status === 'online') {
            await db_config_1.connectionPool.query('UPDATE chargers SET network_status = ?, status = ? WHERE id = ?', ['offline', 'powered_off', chargerId]);
            // Notificar cambio de estado por WebSocket
            app_1.webSocketServer.notifyStatusChange(Number(chargerId), 'powered_off', 'offline');
        }
        res.json({
            success: true,
            isAlive,
...
// --- UNLINK ---
router.post('/:chargerId/factory/unlink', auth_1.authenticate, async (req, res) => {
    const chargerId = req.params.chargerId;
    ...
    try {
        ...
        const [exists] = await db_config_1.connectionPool.query('SELECT * FROM charger_users WHERE charger_id = ? AND user_id = ?', [chargerId, userId]);
        ...
        const [result] = await db_config_1.connectionPool.query('DELETE FROM charger_users WHERE charger_id = ? AND user_id = ?', [chargerId, userId]);
        console.log(`[UNLINK] DELETE charger_users result:`, result);
        // Verificar si quedan usuarios
        const [rows] = await db_config_1.connectionPool.query('SELECT COUNT(*) as count FROM charger_users WHERE charger_id = ?', [chargerId]);
        console.log(`[UNLINK] Usuarios restantes para cargador ${chargerId}:`, rows[0]);
        if (rows[0].count === 0) {
            const [delResult] = await db_config_1.connectionPool.query('DELETE FROM chargers WHERE id = ?', [chargerId]);
...
router.put('/:chargerId/users/:userId/price', auth_1.authenticate, async (req, res) => {
    const chargerId = req.params.chargerId;
    ...
    try {
        ...
        const [userRows] = await db_config_1.connectionPool.query('SELECT access_level FROM charger_users WHERE charger_id = ? AND user_id = ?', [chargerId, userId]);
        ...
        const [baseRows] = await deviceDb_config_1.deviceDbPool.query('SELECT base_price_per_kwh FROM administracion.pricing_devices WHERE serial_number = ?', [serial]);
        ...
        // Actualizar precio
        await db_config_1.connectionPool.query('UPDATE charger_users SET rate_per_kwh = ? WHERE charger_id = ? AND user_id = ?', [rate_per_kwh, chargerId, userId]);
...
router.get('/pricing/:serial_number', async (req, res) => {
    ...
    try {
        const [rows] = await deviceDb_config_1.deviceDbPool.query(`SELECT base_price_per_kwh, price_grid, price_solar, price_mixed, price_surplus FROM administracion.pricing_devices WHERE serial_number = ?`, [serial_number]);
        if (!rows || rows.length === 0) {
            return res.status(404).json({ error: 'Cargador no encontrado' });
        }
        res.json(rows[0]);
...
router.put('/:id/usage-type', auth_1.authenticate, async (req, res) => {
    if (!req.user || !req.user.id) {
        return res.status(401).json({ success: false, error: 'Usuario no autenticado' });
    }
    const { id } = req.params;
    ...
    try {
        ...
        const [chargerRows] = await conn.query(`SELECT c.id FROM chargers c 
       JOIN charger_users cu ON c.id = cu.charger_id 
       WHERE c.id = ? AND cu.user_id = ?`, [id, req.user.id]);
        if (chargerRows.length === 0) {
            return res.status(404).json({ success: false, error: 'Cargador no encontrado o no autorizado' });
        }
        // Actualizar el tipo de uso
        await conn.query('UPDATE chargers SET usage_type = ? WHERE id = ?', [usage_type, id]);
...
router.get('/home-chargers', auth_1.authenticate, async (req, res) => {
    if (!req.user || !req.user.id) {
        return res.status(401).json({ success: false, error: 'Usuario no autenticado' });
    }
    let conn;
    try {
        ...
        const [chargerRows] = await conn.query(`SELECT c.id, c.name, c.serial_number, c.created_at 
       FROM chargers c 
       JOIN charger_users cu ON c.id = cu.charger_id 
...
router.get('/api/ocpp-servers', async (req, res) => {
    try {
        const [servers] = await deviceDb_config_1.deviceDbPool.query('SELECT * FROM ocpp_servers ORDER BY name');
        res.json(servers);
    }
    catch (error) {
