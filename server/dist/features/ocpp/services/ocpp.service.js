...
exports.ocppService = exports.OCPPService = void 0;
...
class OCPPService {
    ...
    async resetAllChargersStatus() {
        try {
            console.log('[OCPP] Reseteando estado de conexion de todos los cargadores a offline...');
            await db_config_1.connectionPool.query("UPDATE chargers SET network_status = 'offline' WHERE network_status = 'online'");
    ...
    getChargerConnection(chargePointId) {
        const connection = this.connectedChargers.get(chargePointId);
        if (!connection)
            return null;
        const ip = connection.ip;
        const isLocalWiFi = ip && (ip.startsWith('192.168.') ||
            ip.startsWith('10.') ||
            (ip.startsWith('172.') &&
                parseInt(ip.split('.')[1]) >= 16 &&
                parseInt(ip.split('.')[1]) <= 31));
        // Check for common VPN and internet IP types
        const isVPN = ip && (ip.startsWith('172.') || // Common VPN range
     ...
     */
    updateChargerHeartbeat(chargePointId) {
    ...
    async registerCharger(chargePointId, wsConnection) {
        ...
        const [rows] = await db_config_1.connectionPool.query('SELECT * FROM chargers WHERE serial_number = ?', [chargePointId]);
        if (rows.length === 0) {
        ...
        const isLocalWiFi = ip && (ip.startsWith('192.168.') ||
            ...
            (ip.startsWith('172.') &&
                parseInt(ip.split('.')[1]) >= 16 &&
                parseInt(ip.split('.')[1]) <= 31));
        const isVPN = ip && (ip.startsWith('172.') ||
            ip.startsWith('10.') ||
            ip.includes('vpn') ||
            ip.includes('tun') ||
            ip.includes('ppp'));
        ...
        try {
            await db_config_1.connectionPool.query(`UPDATE chargers SET 
                 ...
                 last_updated = NOW()
                 WHERE serial_number = ?`, [chargePointId]);
    ...
    async unregisterCharger(chargePointId) {
        console.log(`[OCPP] Desregistrando cargador ${chargePointId}`);
        this.connectedChargers.delete(chargePointId);
        try {
            await db_config_1.connectionPool.query("UPDATE chargers SET network_status = 'offline', last_updated = NOW() WHERE serial_number = ?", [chargePointId]);
    ...
    isChargerConnected(chargePointId) {
...
exports.ocppService = new OCPPService();
