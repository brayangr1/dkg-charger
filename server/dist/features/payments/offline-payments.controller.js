...
exports.processOfflinePayment = void 0;
...
const processOfflinePayment = async (req, res) => {
    const connection = await db_config_1.connectionPool.getConnection();
    try {
        await connection.beginTransaction();
        const { sessionId, chargerId, userId, amount, energy, duration, paymentMethodId, offlineInvoiceId } = req.body;
        ...
        const [existingPayments] = await connection.query('SELECT * FROM payments WHERE session_id = ? AND user_id = ?', [sessionId, userIdToUse]);
        ...
        try {
            const [pmRows] = await connection.query('SELECT id FROM payment_methods WHERE id = ?', [methodIdToUse]);
            if (!pmRows || pmRows.length === 0) {
                ...
                await connection.query(`INSERT INTO payment_methods (id, user_id, type, card_brand, last4, is_default, created_at) VALUES (?, ?, 'card', 'offline', '0000', 0, NOW())`, [methodIdToUse, userIdToUse]);
            }
        }
        catch (pmErr) {
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`, [
        ...
        if (isNumericSession) {
            ...
            const [existingSessions] = await connection.query('SELECT id FROM charging_sessions WHERE id = ?', [sessionDbId]);
            if (!existingSessions || existingSessions.length === 0) {
                const [res] = await connection.query(`INSERT INTO charging_sessions 
...
exports.processOfflinePayment = processOfflinePayment;
