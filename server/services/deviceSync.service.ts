 ...
 */
export async function syncChargerData(serial: string) {
  ...
  const connDevice = await deviceDbPool.getConnection();
  const connMain = await connectionPool.getConnection();

  try {
    // 🔄 Obtener registros de carga del equipo
    const [logRows] = await connDevice.query<RowDataPacket[]>(`SELECT * FROM \`${logTable}\``);
    ...
    const [actionRows] = await connDevice.query<RowDataPacket[]>(`SELECT * FROM \`${actionTable}\``);

    for (const act of actionRows) {
      await connMain.query(
        `INSERT INTO charger_logs (charger_id, action, description, executed_at)
         VALUES (?, ?, ?, ?)`,
        [
          chargerId,
          act.action_type,
    ...
    const [lastActionRows] = await connDevice.query<RowDataPacket[]>(
    ...
    if (lastActionRows.length > 0) {
      ...
      if (lastActionRow.status !== undefined) {
        ...
        const [existingRow] = await connMain.query<RowDataPacket[]>(
 ...
 */
export async function getAllDeviceSerials(): Promise<string[]> {
  const conn = await deviceDbPool.getConnection();
  try {
    const [rows] = await conn.query<RowDataPacket[]>('SELECT serial FROM devices');
