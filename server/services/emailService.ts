...
const generateReceiptHTML = (payment: PaymentHistory & { billing?: any }): string => {
...
export const sendPaymentReceiptEmail = async (
  ...
  try {
    ...
    return await sendEmail(
      userEmail,
      `Recibo de Pago - ${payment.invoice_number}`,
      htmlContent
    );
  } catch (error) {
...
export const sendPaymentConfirmationEmail = async (
  ...
  try {
    ...
    return await sendEmail(
      userEmail,
      `Pago Confirmado - ${formatAmount(payment.amount, payment.currency)}`,
      html
    );
  } catch (error) {
