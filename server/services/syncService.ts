import { deviceDbPool, connectionPool } from '../config/db.config';
import { ChargerStatus } from '../features/chargers/charger.model';
import cron from 'node-cron';
import { RowDataPacket } from 'mysql2/promise';
import { getAdminConnection } from '../config/adm.config';
import { webSocketServer } from '../src/app'; // Importar WebSocketServer


class SyncService {
  private static instance: SyncService;
  private syncInterval = '*/5 * * * *'; // Cada 5 minutos
  private priceSyncInterval = '*/1 * * * *'; // Cada día a las 3 AM
  private lastStatuses: Map<number, string> = new Map(); // Guardar último estado por chargerId
...
/*
  ...
  private async syncActionTable(chargerId: number, actionTable: string) {
    try {
      // Verificar si la tabla existe
      const [tableExists] = await deviceDbPool.query<RowDataPacket[]>(
        `SHOW TABLES LIKE ?`, 
        [actionTable]
      );
      ...
      const [columns] = await deviceDbPool.query<RowDataPacket[]>(
        `SHOW COLUMNS FROM \`${actionTable}\``
      );
      ...
      const [lastAction] = await deviceDbPool.query<RowDataPacket[]>(
      ...
      const [lastPrimary] = await connectionPool.query<RowDataPacket[]>(
        `SELECT action_type, id_user, id_device, description, status, executed_at 
      ...
      if (shouldNotify) {
        ...
        let statusText: ChargerStatus = 'standby';
        switch (action.status) {
          case '0': statusText = 'standby'; break;
          case '1': statusText = 'locked'; break;
          case '2': statusText = 'charging'; break;
          case '3': statusText = 'error'; break;
          case '4': statusText = 'offline'; break;
          default: statusText = 'standby';
        }
        
        // Notificar a través de WebSocket
        webSocketServer.notifyStatusChange(chargerId, statusText, 'online');
  ...
  private async monitorActiveCharges() {
    try {
      const [chargingChargers] = await connectionPool.query<RowDataPacket[]>(
        `SELECT 
      ...
      for (const charger of chargingChargers) {
        const [deviceDetails] = await deviceDbPool.query<RowDataPacket[]>(
          'SELECT power, temperature FROM details WHERE device_id = (SELECT id FROM devices WHERE serial = ?)',
  ...
  private async checkPrimaryDevicesStatusChanges() {
    try {
      // Obtener todos los registros más recientes de primary_devices
      const [latestStatuses] = await connectionPool.query<RowDataPacket[]>(
        `SELECT pd.id_device, pd.status, pd.executed_at, c.id as charger_id
...
/*
  ...
  private async syncDevice(serial: string) {
    try {
      const [deviceData] = await deviceDbPool.query<RowDataPacket[]>(
        `SELECT * FROM details WHERE device_id = (SELECT id FROM devices WHERE serial = ?)`,
        [serial]
      );
      ...
      if (data.temperature > 60) {
        ...
        if (chargerRow.length > 0) {
          ...
          await connectionPool.query(
            ...
            [chargerId, 'alert', `Sobrecalentamiento detectado: ${data.temperature}°C (syncDevice)`]
          );
        }
      }
      ...
      if (chargerRows.length > 0) {
        ...
        let statusNumber;
        switch (newStatus) {
          case 'standby': statusNumber = '0'; break;
          case 'locked': statusNumber = '1'; break;
          case 'charging': statusNumber = '2'; break;
          case 'error': statusNumber = '3'; break;
          case 'offline': statusNumber = '4'; break;
        ...
        if (existingRow.length === 0) {
          await connectionPool.query(
            `INSERT INTO primary_devices (action_type, id_user, id_device, description, status, executed_at) 
             VALUES (?, ?, ?, ?, ?, NOW())`,
            ['status_update', 0, chargerId, `Estado actualizado a ${newStatus}`, statusNumber]
          );
        }
      }
      ...
      await connectionPool.query(
        `UPDATE chargers SET network_status = 'offline' WHERE serial_number = ?`,
        [serial]
      );
    }
  }

  private determineStatus(data: any): ChargerStatus {
    if (data.power > 100) return 'charging';
    if (data.temperature > 60) return 'error';
    return 'standby';
  }
}
